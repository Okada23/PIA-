...
def seleccionar_fecha_reservacion():
    while True:
        fecha_reserva = input("\nIngrese la fecha de la reserva (mm-dd-aaaa): ").strip()
    ...
    try:
        with sqlite3.connect ("ReservasCoworking.db") as conn:
            ...
            while True:
                ...
                respuesta_turno = input("\nIngrese la clave del turno (o escriba 'BACK' para volver): ").strip().upper()

                if respuesta_turno == "BACK":
...
def seleccionar_sala(fecha_reserva, turno_seleccionado):
    ...
    try:
        with sqlite3.connect("ReservasCoworking.db") as conn:
            ...
            print("\n" + "=" * 50)
            print(f"SALAS DISPONIBLES el {fecha_reserva.strftime('%d-%m-%Y')} en turno {turno_seleccionado}")
            print("=" * 50)
